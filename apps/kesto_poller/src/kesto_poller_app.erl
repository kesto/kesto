...
start(_StartType, _StartArgs) ->
	riak_core_util:start_app_deps(kesto_poller),

    %% Append user-provided code paths
    case app_helper:get_env(kesto_poller, add_paths) of
    ...
    catch cluster_info:register_app(kesto_poller_cinfo),

    %% Spin up supervisor
    case kesto_poller_sup:start_link() of
        {ok, Pid} ->
            ok = riak_core:register(kesto_poller, [
                {vnode_module, kesto_poller_vnode}
            ]),
			ok = riak_core_node_watcher:service_up(kesto_poller, self()),
            {ok, Pid};
        {error, Reason} ->
            {error, Reason}
    end.
