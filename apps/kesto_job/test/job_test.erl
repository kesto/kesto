...
job_test_() ->
	{foreach,
	 ...
	 cleanup(),
	 [
	  {timeout, 60, [fun run_simple1/0]},
	  {timeout, 60, [fun run_simple2/0]},
	  {timeout, 60, [fun run_simple3/0]},
	  {timeout, 60, [fun run_simple4/0]},
	  {timeout, 600, [fun run_simple5/0]}
	 ]
	}.
-endif.
...
run_simple3() ->
	...
	Net1 = #job_conf{id="net1", 
					 ...
					 group_id="group1", 
					 parent_id="root", 
					 child_id=[{"group1", "net2"}, {"group1", "job2"}], 
					 previous_id=[], 
					 next_id=[], 
					 info_end_value_from=0,
					 info_end_value_to=0,
...
run_simple5() ->
	...
	Net1 = #job_conf{id="net1", 
					 ...
					 group_id="group1", 
					 parent_id="root", 
					 child_id=[{"group1", string:join(["job", erlang:integer_to_list(X)], "")} || X <- lists:seq(1, Max)], 
					 previous_id=[], 
					 next_id=[], 
					 info_end_value_from=0,
					 info_end_value_to=0,
	...
	Job1 = #job_conf{id="job1", 
					 ...
					 command=#job_conf_command{cmd_facility_id="scope1", 
											   ...
											   cmd_effective_user="root", 
											   cmd_error_end_flg=false}},
	make_job_conf(Job1, "job" , lists:seq(1, Max)),
...
make_job_conf(_, _, []) ->
	ok;
make_job_conf(BaseConf, BaseJobID, [NumList | T]) ->
	Conf = BaseConf#job_conf{id=string:join([BaseJobID, erlang:integer_to_list(NumList)], "")},
	?debugVal(Conf),
	ok = kesto_job_conf:put(Conf),
	make_job_conf(BaseConf, BaseJobID, T).