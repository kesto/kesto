...
%%
%%<dl><dt> raw_name
%%</dt><dd>   the base path under which the kesto_core_wm_raw
%%            should be exposed; defaulted to "raw"
%%</dd></dl>

-module(kesto_core_web).
...
dispatch_table() ->
	lists:append(raw_dispatch(),
				 [
				  {["ping"], kesto_core_wm_ping, []}
				 ]).

raw_dispatch() ->
	case app_helper:get_env(kesto_core, raw_name) of
...
raw_dispatch(Name) ->
	...
	[
	 {["repository", 'repository', "keys", 'key'],
	  kesto_core_wm_repository, Props1}
	].
