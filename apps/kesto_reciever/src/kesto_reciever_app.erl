...
start(_StartType, _StartArgs) ->
	riak_core_util:start_app_deps(kesto_reciever),

    %% Append user-provided code paths
    case app_helper:get_env(kesto_reciever, add_paths) of
    ...
    catch cluster_info:register_app(kesto_reciever_cinfo),

    %% Spin up supervisor
    case kesto_reciever_sup:start_link() of
        {ok, Pid} ->
            ok = riak_core:register(kesto_reciever, [
                {vnode_module, kesto_reciever_vnode}
            ]),
			ok = riak_core_node_watcher:service_up(kesto_reciever, self()),
            {ok, Pid};
        {error, Reason} ->
            {error, Reason}
    end.
