...
start(_StartType, _StartArgs) ->
	riak_core_util:start_app_deps(kesto_control),

    %% Append user-provided code paths
    case app_helper:get_env(kesto_control, add_paths) of
    ...
    catch cluster_info:register_app(kesto_control_cinfo),

    %% Spin up supervisor
    case kesto_control_sup:start_link() of
        {ok, Pid} ->
%%             ok = riak_core:register(kesto_control, [
%% 												{vnode_module, kesto_control_vnode}
%% 											   ]),
			ok = riak_core_node_watcher:service_up(kesto_control, self()),
            {ok, Pid};
        {error, Reason} ->
            {error, Reason}
    end.
